...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
	...
	if (tex.rgb == vec3(0.0)){
		...
		vec3 col1 = vec3(31.0, 36.0, 99.0)/255.0;
		vec3 col2 = vec3(4.0, 12.0, 126.0)/255.0;

		float X = mix(uv.x, 0.5, pow(abs(uv.y - 0.5), 2.0));
		float circ = sqrt(-pow((2.0*uv.x - 1.0), 2.0) + 1.0);
		float t = X*5.0 + jevil.y * 2.5;
		float value = mod(t, 1.0);
		vec3 col = mix(col1, col2, step(value, 0.5));
		col = mix(col, col * vec3(0.75), pow(0.5 + abs(uv.x - 0.5), 2.0));
		col = mix(col, col * vec3(0.75), 1.0 - pow(0.5 + abs(uv.y - 0.5), 2.0));
		tex.rgb = col;
