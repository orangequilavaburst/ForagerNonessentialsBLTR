...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    // pixelate
    float mosiac_size = 0.25;
    vec2 mosiac_mult = texture_details.zz / mosiac_size;
    vec2 sprite_dim = texture_details.zw / image_details;
	vec2 sprite_pos = texture_details.xy * sprite_dim;
    vec2 new_coords = clamp((floor(texture_coords * mosiac_mult) + vec2(mod(mosiac.x, 0.5) + 0.25))/ mosiac_mult, sprite_dim, sprite_dim + sprite_pos);
