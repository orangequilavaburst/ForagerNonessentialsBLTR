...
{   
    float absX = abs(uv.x);
    uv.y = -uv.y * 1.2 + absX * absX * 1.1 - absX * 0.675;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
	...
	if (tex.rgb == vec3(0.0)){
	    float d = fract(heart((uv - vec2(0.5)) * .4) * 5. - yuri.y);
		vec3 f = d < 0.8 ? vec3(216.0/255.0, 123.0/255.0, 65.0/255.0) : vec3(228.0/255.0, 57.0/255.0, 49.0/255.0);
		f = d < 0.6 ? vec3(1.0) : f ;
		f = d < 0.4 ? vec3(186.0/255.0, 96.0/255.0, 120.0/255.0) : f;
		f = d < 0.2 ? vec3(167.0/255.0, 74.0/255.0, 120.0/255.0) : f;
		tex.rgb = f;
	}
