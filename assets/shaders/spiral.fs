...
float spiral_value(vec2 m) {
    float t = spiral.y * 0.125;
	float r = length(m);
	float a = atan(m.y, m.x);
	float v = sin(100.*(sqrt(r)-0.02*a-.3*t));
	return step(0.5, clamp(pow(v, 2.0),0.,1.));
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
	...
	float v = spiral_value((uv - vec2(0.5))*0.125);
	if (tex.rgb == vec3(0.0)){
	    tex.rgb = mix(vec3(v), RGB(vec4(spiral.y*0.25, 0.9, 0.8, 1.0)).rgb, RGB(vec4(spiral.y*0.25 + 0.125, 0.9, 0.8, 1.0)).rgb);
	
		float ct = length(abs(uv - vec2(0.5)));
		float ctcol = ct;
		ct = pow(ct, 3.0);
		ctcol = 1.0-pow(1.0-ct, 5.0);
		
		vec4 huetex = HSL(tex);
		vec4 newtex = RGB(vec4(huetex.r + 0.25, huetex.g, huetex.b*0.5, huetex.a));
		tex.rgb = mix(tex.rgb, newtex.rgb, min(1.0, ctcol + 0.1));
	}
