...
extern PRECISION vec2 scroll;
extern PRECISION vec2 scroll_speed;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    vec2 new_coords = mod(texture_coords + scroll_speed*scroll.y, vec2(1.0));
	// Take pixel color (rgba) from `texture` at `texture_coords`, equivalent of texture2D in GLSL
    vec4 tex = Texel(texture, new_coords);

    // Position of a pixel within the sprite
	vec2 uv = (((texture_coords)*(image_details)) - texture_details.xy*texture_details.ba)/texture_details.ba;
    tex.a = (uv.x < 0.02 || uv.x > 0.98 || uv.y < 0.02 || uv.y > 0.98) ? 0.0 : step(0.25, tex.a);

    // required
	return dissolve_mask(tex*colour, new_coords, uv);
}
